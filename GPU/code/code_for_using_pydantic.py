...
class GPU(BaseModel):
    ...
    gpuInterconnection: Optional[GpuInterconnection] = Field(
        None,
        description="Interconnection of multiple GPUs within a server systemEnum:'NVLink', 'RoCE2', 'Xe Link', 'Infinity Fabric', 'other', 'none'",
    )
    gpuMemory: Optional[float] = Field(None, description='Size of memory of the GPU.')
    gpuPassthrough: Optional[bool] = Field(
        None,
        description='If true, GPU is provided via passthrough, otherwise GPU is virtualized.',
    )
    gpuProcessingUnits: Optional[confloat(ge=1.0)] = Field(
        None,
    ...
    type: Optional[Type6] = Field(
        None, description='NGSI entity type. It has to be GPU'
    )
